...
description: Use when asked to draw excalidraw diagram
---
...
## Style selection (do this first)

Ask: **who is this diagram for?**

| Audience | Mode | What to read |
|----------|------|--------------|
| Someone else / teaching / docs | **Default** (this file) | Continue below; use `references/color-palette.md` |
| The user themselves, mid-design | **Sketch** | **REQUIRED:** read `references/sketch-style.md` (and `references/sketch-element-templates.md` for shapes). Skip evidence artifacts, titles, and comprehensive teaching structure. |

Prefer sketch when the user asks to sketch, whiteboard, or quickly diagram their own design/thinking — even without the word "sketch." Prefer default when explaining a finished concept to a reader with zero context.

## Customization

**Default mode colors:** `references/color-palette.md`. Read it before generating any default-mode diagram and use it as the single source of truth for color choices — shape fills, strokes, text colors, evidence artifact backgrounds, everything.

**Sketch mode colors:** defined in `references/sketch-style.md` (small fixed palette). Do not use `color-palette.md` for sketch diagrams.

To rebrand default-mode diagrams, edit `color-palette.md`. Everything else in this file is universal design methodology and Excalidraw best practices.
...
### Roughness
- `roughness: 0` — Clean, crisp edges. Use for modern/technical diagrams (default mode).
- `roughness: 1` — Hand-drawn, organic feel. Use for sketch mode — see `references/sketch-style.md`.

**Default to 0** unless sketch mode applies.
...
If `uv` itself isn't installed (`command not found: uv`), install it first — e.g. on macOS,
`brew install uv` — then re-run the two commands above.

### Troubleshooting: render hangs on `Page.wait_for_function: Timeout ... window.__moduleReady`

This means `render_template.html`'s `import ... from "https://esm.sh/@excalidraw/excalidraw..."`
never resolved in the headless browser — almost always an esm.sh CDN issue with that specific
package build, not a problem with your `.excalidraw` JSON or your network. Confirmed once
(2026-08-12): the unpinned/"latest" resolution pulled in `@excalidraw/mermaid-to-excalidraw`,
which in turn 404'd on `@braintree/sanitize-url`'s `es2022/dist/constants.mjs` — a broken deep
import on esm.sh's side. `render_template.html` now pins `@excalidraw/excalidraw@0.18.0`, whose
bundle doesn't pull that dependency in at all, so this shouldn't recur. If it does (a future pin
breaks the same way), diagnose with this snippet before assuming your JSON is at fault:

```python
from playwright.sync_api import sync_playwright
from pathlib import Path
template = Path("references/render_template.html").resolve()  # or your candidate version
with sync_playwright() as p:
    browser = p.chromium.launch(headless=True)
    page = browser.new_page()
    page.on("console", lambda msg: print("CONSOLE:", msg.type, msg.text))
    page.on("pageerror", lambda exc: print("PAGEERROR:", exc))
    page.on("requestfailed", lambda req: print("REQFAILED:", req.url))
    page.goto(template.as_uri())
    page.wait_for_timeout(8000)
    print("moduleReady:", page.evaluate("window.__moduleReady"))
    browser.close()
```

A `REQFAILED`/404 on some transitive dependency confirms a CDN-side break. To find a working pin,
probe candidate versions directly: `curl -s "https://esm.sh/@excalidraw/excalidraw@<version>?bundle"`
and check whether the top-level import list drags in the broken package — if it doesn't appear in
that list at all, that version's bundle graph avoids the broken path. Update the pinned version in
`render_template.html` (and note the date/reason in the comment above the import) once verified.
...
18. **Roughness**: `roughness: 0` for default mode; `roughness: 1` + hachure when using sketch style
